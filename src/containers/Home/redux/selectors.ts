...
const getCorporateCommunicationSelector = createSelector(selectGlobalSubStore, (globalState) => {
...
const getApprovalFeatureModulesSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.approvalFeatureModulesData;
});

const getOfferCategoriesSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.offerCategoriesData;
});
const getOffersSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.offersData;
});

const getOffersDetailsSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.offersDetails;
});

const getServiceRequestSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.serviceRequestData;
});

const getUsefulAppsSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.usefullAppsData;
});

const getknowledgeHubCategoriesSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.knowledgeHubCategoriesData;
});

const getFeatureModuleDataSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.featureModuleData;
});

const getQoutesSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.qouteList;
});

const getSurveySelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.surveyList;
});

const getEventsSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.eventsList;
});

const getEventsDetailsSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.eventsDetails;
});

const getDocumentsSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.documentsList;
});

const getDocumentsDetailsSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.documentsDetails;
});

const getAcknowledgeDataSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.acknowledgeData;
});

const getCancelledAcknowledgeDataSelector = createSelector(selectGlobalSubStore, (globalState) => {
  return globalState.cancelledAcknowledgeData;
});

const getOrganizationStructureDataSelector = createSelector(selectGlobalSubStore, (globalState) => {
...
const getUnreadNotificationCountSelector = createSelector(selectGlobalSubStore, (globalState) => {
