...
const selectGlobalSubStore = (store) =>
  store[REDUCER_KEY.KNOWLEDGEHUB_REDUCER] || initialState;

const getKnowledgehubDocumentsSelector = createSelector(
  selectGlobalSubStore,
  (globalState) => {
    return globalState.KnowledgehubDocumentsData;
  }
);
const getKnowledgehubTagSelector = createSelector(
  selectGlobalSubStore,
  (globalState) => {
    return globalState.KnowledgehubTagsData;
  }
);
const getKnowledgeHubLoadingSelector = createSelector(
  selectGlobalSubStore,
  (globalState) => {
